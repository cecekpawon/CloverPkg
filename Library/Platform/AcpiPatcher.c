...
DropTableFromRSDT (
  ...
  CONSTRAIN_MAX (EntryCount, 100); // it's enough
  ...
  for (Index = 0; Index < EntryCount; Index++, EntryPtr++) {
    ...
    /*
          if (
            ((TableEntry->Signature != Signature) && (Signature != 0)) ||
...
PatchACPI (
  ...
  BOOLEAN                                                 DsdtLoaded = FALSE, OSTypeDarwin = FALSE, //, NeedUpdate = FALSE
