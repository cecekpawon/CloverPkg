...
EFI_STATUS ParseTagKey          (CHAR8 *buffer, TagPtr *tag, UINT32 *lenPtr);
...
ParseXML (
  ...
  EFI_STATUS    Status;
  UINT32        length = 0, pos = 0, bufferSize = 0;
  TagPtr        tag = NULL;
...
XMLParseNextTag (
  ...
  *lenPtr = 0;
  ...
  if (!AsciiStrnCmp(tagName, kXMLTagPList, 6)) {
    length=0;
    Status=EFI_SUCCESS;
  } else if (!AsciiStrCmp(tagName, kXMLTagDict)) { /***** dict ****/
...
ParseTagList (
  ...
  UINT32        pos, length = 0;
  TagPtr        tagList, tagTail, tmpTag = NULL;
...
ParseTagKey (
  ...
  EFI_STATUS    Status;
  UINT32        length = 0, length2 = 0;
  CHAR8         *tmpString;
  TagPtr        tmpTag, subTag = NULL;
