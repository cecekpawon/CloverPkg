...
MAP_EFI_GUID_STR AptioEfiGuidStrMap[] = {
...
AptioGuidStr (
  ...
  for (i = 1; AptioEfiGuidStrMap[i].Guid != NULL; i++) {
    if (CompareGuid (AptioEfiGuidStrMap[i].Guid, Guid)) {
      Str = AptioEfiGuidStrMap[i].Str;
      break;
    }
  }

  if (Str == NULL) {
    UnicodeSPrint (AptioEfiGuidStrMap[0].Str, 47 * 2, L"%g", Guid);
    Str = AptioEfiGuidStrMap[0].Str;
  }
...
StrStriBasic (
  ...
  CONST CHAR16 *FirstMatch, *SearchStringTmp;
  ...
  while (*String != L'\0') {
    ...
    while (
      (ToUpperChar (*String) == ToUpperChar (*SearchStringTmp)) &&
      (*String != L'\0')
    ) {
      String++;
      SearchStringTmp++;
...
StriStartsWithBasic (
  ...
  return (((Chr1 == L'\0') && (Chr2 == L'\0')) || ((Chr1 != L'\0') && (Chr2 == L'\0')));
}
#endif
