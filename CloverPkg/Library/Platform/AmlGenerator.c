...
UINT32
aml_fill_simple_name (
  CHAR8     *buffer,
  CHAR8     *name
) {
  if (AsciiStrLen(name) < 4) {
    //    MsgLog("aml_fill_simple_name: simple name %a has incorrect lengh! Must
    //    be 4.\n", name);
    return 0;
  }
...
aml_fill_name (
  ...
  if (((len % 4) > 1) || (count == 0)) {
    //    MsgLog("aml_fill_name: pathname %a has incorrect length! Must be 4, 8,
    //    12, 16, etc...\n", name);
    return 0;
  }
